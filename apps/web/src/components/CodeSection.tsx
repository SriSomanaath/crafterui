...
export function CodeSection({
  ...
  // Arriving at #code (the preview's code control) means "show me the source",
  // so unfold it. A shared link to #code lands the same way.
  useEffect(() => {
    const sync = () => {
      if (window.location.hash === "#code") setOpen(true);
    };
    sync();
      ...
      <section>
            ...
            {MANAGERS.map((m) => (
              <button
                ...
                className={`relative rounded-full px-2.5 py-1 text-xs font-medium select-none transition-[color,background-color,scale] active:scale-[0.96] after:absolute after:inset-x-0 after:-inset-y-2 after:content-[''] ${
                  manager === m.id ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground hover:bg-muted"
                }`}
              >
                {m.label}
              </button>
        ...
        <div className="flex items-stretch gap-2">
          ...
          <button
            ...
            className="grid w-10 shrink-0 place-items-center rounded-lg bg-background text-muted-foreground shadow-border transition-[color,box-shadow,scale] hover:text-foreground hover:shadow-border-hover active:scale-[0.96]"
